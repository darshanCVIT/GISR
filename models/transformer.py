...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_glance_enc_layers=3, num_gaze_s1_dec_layers=3, num_gaze_s1_enc_layers=3, 
        ...
        self.verb_classifier_clip = nn.Sequential(nn.Linear(d_model*3, d_model*3),
                                             nn.ReLU(),
                                             nn.Dropout(0.3),
                                             nn.Linear(d_model*3, self.num_verb_classes))
        self.ln2_clip = nn.LayerNorm(d_model*3)
    ...
    def forward(self, object_features, IL_token_embed, RL_token_embed, verb_token_embed, role_token_embed, object_pos_embed, vidx_ridx, clip_embs, targets=None, inference=False):
        ...
        # import pdb
        # pdb.set_trace()
        #clip_feature = torch.rand(bs, self.d_clip).to(device)
        clip_feature = self.project_clip(clip_embs)
        vhs = torch.cat([IL_token_feature, RL_token_feature, clip_feature], dim=-1)
        vhs = self.ln2_clip(vhs)
        verb_pred = self.verb_classifier_clip(vhs).view(bs, self.num_verb_classes)
