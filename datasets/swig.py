...
def collater(data):
    ...
    #TODO
    #clip_embs = [torch.from_numpy(s['clip']) for s in data]
    clip_embs = np.stack([s['clip'] for s in data],axis=0)
    clip_embs = torch.tensor(clip_embs).float()
...
class Resizer(object):
    ...
    def __call__(self, sample, min_side=512, max_side=700):
        ...
        return {'img': torch.from_numpy(new_image), 'annot': torch.from_numpy(annots), 'scale': scale, 'img_name': image_name, 'clip':sample['clip'], 'verb_idx': sample['verb_idx'], 'verb_role_idx': sample['verb_role_idx'], 'shift_1': shift_1, 'shift_0': shift_0}
...
class Augmenter(object):
    def __call__(self, sample, flip_x=0.5):
        ...
        sample = {'img': image, 'annot': annots, 'img_name': img_name, 'clip':sample['clip'], 'verb_idx': sample['verb_idx'], 'verb_role_idx': sample['verb_role_idx']}
...
class Normalizer(object):
    ...
    def __call__(self, sample):
        ...
        return {'img': ((image.astype(np.float32) - self.mean) / self.std), 'annot': annots, 'clip':sample['clip'], 'img_name': sample['img_name'], 'verb_idx': sample['verb_idx'], 'verb_role_idx': sample['verb_role_idx']}
